...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			function scrollVSetting():void
			{
				...
				else if (y < 0 - _yOverlap)
				{
					if (_isStickTouch)
						...
						_maskContentHolder.y = (y + 0 - _yOverlap) * 0.5;
				}
				else
				{
			...
			function scrollHSetting():void
			{
				...
				else if (x < 0 - _xOverlap)
				{
					if (_isStickTouch)
						...
						_maskContentHolder.x = (x + 0 - _xOverlap) * 0.5;
				}
				else
				{
		...
		public function mouseUpHandler(e:MouseEvent = null):void
		{
			...
			if (!_doMouseUpScroll)
				return; // if it was false pause everything untill the user himself set it to true manually again
		...
		private function doMouseUpAni():void
		{
			...
			var tolerance:Object = {minDuration: (_isStickTouch) ? 0 : .3, overShoot: (_isStickTouch) ? 0 : 1};

			ThrowPropsPlugin.to(_maskContentHolder, {throwProps: {y: {velocity: yVelocity, max: _maskHolder.y, min: _maskHolder.y - _yOverlap, resistance: 300}, x: {velocity: xVelocity, max: _maskHolder.x, min: _maskHolder.x - _xOverlap, resistance: 300}}, onUpdate: onTweenUpdate, onComplete: onTweenComplete, ease: _easeTypeFunc}, 10, tolerance.minDuration, tolerance.overShoot);
		}
		...
		protected function onTweenComplete():void
		{
			...
			if (_bitmapMode == ScrollConst.NORMAL || _bitmapMode == ScrollConst.WEAK)
		...
		private function setYPercAndSpeed():void
		{
			...
			if (_maskContentHolder.y > 0)
				currY = 0; // if touch scroll was scratching at start ponint, set currY to 0 obviously
			else if ((-_maskContentHolder.y) > diff)
				currY = diff; // if it was scratching at end ponint, set currY to diff obviously
			...
			if (blurEffect && aniInterval != 0)
				TweenMax.to(_maskContentHolder, aniInterval, {blurFilter: {blurY: speed}, ease: _easeTypeFunc});
		}
		...
		private function setXPercAndSpeed():void
		{
			...
			if (_maskContentHolder.x > 0)
				currX = 0; // if touch scroll was scratching at start ponint, set currY to 0 obviously
			else if ((-_maskContentHolder.x) > diff)
				currX = diff; // if it was scratching at end ponint, set currY to diff obviously

			// if content is smaller than the mask, so we should not set the percent for no resean and change its default value obviously
			if (_hScroll)
				_xPerc = currX * 100 / diff; // Periodic Table-> diff / 100 = currY / ?
			...
			if (blurEffect && aniInterval != 0)
				TweenMax.to(_maskContentHolder, aniInterval, {blurFilter: {blurX: speed}, ease: _easeTypeFunc});
		}
